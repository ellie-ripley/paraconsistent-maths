...
- subDMQ is a very close relative of the book's logic subDLQ. Differences:
  - removes bad distribution axiom
...
- Differences from Weber:
  - bad distribution form removed
  - axiom refl: provable in the set theory but not in subDLQ; not distinctly set-theoretic
  - axiom efq_entl: provable in the set theory but not in subDLQ; not distinctly set-theoretic and needed (or its implication strength, anyway) for some results of Ch.4
  - axiom equals_sub_impl: implication strength of a subDLQ rule; needed for some results of Ch.4, and for deduction theorem
  - axiom bisub_impl: implication strength of a subDLQ rule; needed for deduction theorem (and some results of ch5?)
- There may also be some differences with the quantifiers, owing to Isabelle's way of handling variables and binding. I don't know that this ramifies, and it all seems to work fine, but I'm not 100λ confident that this *doesn't* ramify.
** Isabelle
#+begin_src isabelle
axiomatization
...
- probably some automation possible here?
...
*** comments
- this late because these depend on some of what's before
...
#+end_src
** Bad distribution
- Email from Libor 2025-3-31: notes that the bad distribution is still derivable; dropping the axiom didn't fix it!
- Q: what to do? Drop one of the De Morgan principles? But what about double negation and contraposition; are those alone enough to force the De Morgans?
*** notlemma bad_dist: "A ∨ (B ⊗ C) ↔ (A ∨ B) ⊗ (A ∨ C)"
- uses dist_cd_bi, the old entailment-strength distribution axiom
- not exported because source block not labelled isabelle, so doesn't interfere
#+begin_src
lemma bad_dist: "A ∨ (B ⊗ C) ↔ (A ∨ B) ⊗ (A ∨ C)"
  proof -
    from dist_cd_bi have
      "(¬A ⊗ ¬B) ∨ (¬A ⊗ ¬C) ↔ ¬A ⊗ (¬B ∨ ¬C)"
      by(rule bientl_sym_rule)
    ...
    from dm_cnnd_bi and this have
      "¬((A ∨ B) ⊗ (A ∨ C)) ↔ ¬(A ∨ (B ⊗ C))"
      by(rule bisub_rule)
    from bientl_contra_impl and this have
      "¬¬(A ∨ (B ⊗ C)) ↔ ¬¬((A ∨ B) ⊗ (A ∨ C))" ..
    from dn_bi and this have
      "(A ∨ (B ⊗ C)) ↔ ¬¬((A ∨ B) ⊗ (A ∨ C))"
      by(rule bisub_rule)
