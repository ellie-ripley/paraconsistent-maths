...
lemma plus_comm: "N x ⊗ N y ⇛ x + y = y + x"
proof -
  from plus_zero and plus_zero' have "N y ⇛ N y ⇛ 0 + y = y + 0"
    by(rule eqsub_rule'_dt)
  from conj_import and this have "N y ⊗ N y ⇛ 0 + y = y + 0"..
  from number_contracts and this have zerocase:"N y ⇛ 0 + y = y + 0"..
  { fix z
    have "(N y ⇛ z + y = y + z) ⇛ N y ⇛ S (z + y) = S (y + z)"
      by(rule eqsub_context_dt)
    from plus_succ and this have "N y ⊗ N z ⇛ (N y ⇛ z + y = y + z) ⇛ (N y ⇛ S (z + y) = y + S z)"
      by(rule eqsub_rule'_dt)
    from plus_succ and this have "N z ⊗ N y ⇛ N y ⊗ N z ⇛ (N y ⇛ z + y = y + z) ⇛ N y ⇛ z + S y = y + S z"
      by(rule eqsub_rule'_dt)
    from conj_import and this have
      "(N z ⊗ N y) ⊗ N y ⊗ N z ⇛ (N y ⇛ z + y = y + z) ⇛ N y ⇛ z + S y = y + S z"..
    from plus_comm_aux and this have
  ...
  have "\<And> z. N z ⇛ (N y ⇛ z + y = y + z) ⇛ N y ⇛ S z + y = y + S z" by fact
  then have inductive:"∀(λ z. N z ⇛ (N y ⇛ z + y = y + z) ⇛ N y ⇛ S z + y = y + S z)" ..
